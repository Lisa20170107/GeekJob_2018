...
function update_profile($name, $birthday, $type, $tell, $comment,$id){
  //db接続を確立
  $update_db = connect2MySQL();
  //DBに全項目のある1レコードを登録するSQL
  $update_sql = "UPDATE user_t SET name = :name,birthday=:birthday,tell=:tell,type=:type,comment=:comment,newDate=:newDate
                 WHERE userID= :id";
...
function serch_profiles($name=null,$year=null,$type=null){
    ...
    if(isset($year) && $flag = false){
        $search_sql .= " AND WHERE birthday LIKE :year";
        $flag = true;
    }else if(isset($year)){
        $search_sql .= " AND birthday LIKE :year";
    }
    if(isset($type) && $flag = false){
        $search_sql .= "AND WHERE type = :type";
    }else if(isset($type)){
